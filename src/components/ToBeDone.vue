...
#ToBeDone(class="px-5 mt-6")
  p(class="px-1 text-[17px] font-bold text-slate-800 tracking-tight flex items-center gap-2 mb-4") 待办
    span(v-if="!isEmpty(data)" class="flex h-2 w-2 relative")
      span(class="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75")
      span(class="relative inline-flex rounded-full h-2 w-2 bg-red-500")
  div(v-if="!isEmpty(data)" class="snap-center shrink-0 w-full sm:w-[340px] relative overflow-hidden rounded-3xl bg-white p-6 shadow-card hover:shadow-card-hover border border-slate-100 transition-all duration-300 active:scale-[0.99]")
    ...
    van-button(class="w-full mt-4" type="primary" class="rounded-xl") 立即查看
