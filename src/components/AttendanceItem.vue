  ...
  van-row(:gutter="0")
    van-col(:span="5")
      div(class="flex flex-col items-center justify-center bg-slate-50 border border-slate-100 rounded-xl w-[3.25rem] h-[3.25rem] shrink-0 text-slate-900 shadow-sm")
        p(class="text-[10px] font-bold uppercase text-slate-400 tracking-wide" v-if="item.attendanceDate") {{dateUtil.getDayOfWeek(item.attendanceDate, {format: 'short'})}}
        p(class="text-[19px]  leading-none mt-1" v-if="item.attendanceDate") {{dateUtil.format(item.attendanceDate, 'DD')}}
    van-col(:span="18")
      div()
  ...
  div(class="mt-4 bg-slate-50/50 rounded-xl pl-6 pr-3 py-3 border border-slate-100/50 relative")
    ...
    div(class="flex items-center mb-4")
      ...
      template(v-if="item.attendanceStartDatetime")
        span(class="text-slate-900 text-[13px] font-semibold" ) {{dateUtil.formatTime(item.attendanceStartDatetime, 'HH:mm')}}
        span(class="text-slate-500 text-xs ml-4") {{t('attendanceSignIn')}}
      template(v-else)
        span(class="text-slate-500 text-[13px] font-semibold" ) --:--
        span(class="text-slate-500 text-xs ml-4" ) {{t('notSignedIn')}}
    div(class="flex items-center")
      span(class="inline-block w-2 h-2 rounded-full bg-white border-[2.5px] border-primary shadow-sm z-10 -ml-[0.4rem] mr-3")
      template(v-if="item.attendanceEndDatetime")
        span(class="text-slate-900 text-[13px] font-semibold") {{dateUtil.formatTime(item.attendanceEndDatetime, 'HH:mm')}}
        span(class="text-slate-500 text-xs ml-4") {{t('attendanceSignOut')}}
      template(v-else)
        span(class="text-slate-500 text-[13px] font-semibold" ) --:--
        span(class="text-slate-500 text-xs ml-4" ) {{t('notSignedOut')}}
</template>
